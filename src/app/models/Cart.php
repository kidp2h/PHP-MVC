...
class Cart extends Model
{
    ...
    {
        return self::$db->query("INSERT INTO cart_item (`user_id`, `product_id`, `quantity`) VALUES ($user_id, '$product_id', '$quantity')");
    }
    ...
    public function totalPriceOfCart($userId)
    {
        $sql = self::$db->query("SELECT SUM(cart_item.quantity*
         ROUND((product.price*(1 - product_details.discount/100)))) AS totalPrice
        FROM product, product_details , user, cart_item 
        WHERE product.id = cart_item.product_id AND cart_item.user_id = user.id 
        AND product.id = product_details.product_id 
        AND user.id = '$userId' AND product.deleted_at IS NULL");
